...
| `@qa-tester`             | QA testing subagent — executes a single scenario per dispatch (FE via Playwright, BE via HTTP + DB CLI). Implemented as two registered variants (`qa-tester-fe`, `qa-tester-be`) for runtime tool-allowlist isolation; presented as the single logical name everywhere user-facing. | `subagent` | [Guide](docs/plugins/qa.md)                 |
