...
export function readConfigFromEnv(env) {
    ...
    if (typeof env.AV_PANTHEON_NOTIFY_DELAY_MS === "string") {
        ...
        if (raw.length > 0) {
            ...
            if (Number.isFinite(parsed) && parsed >= 0) {
                ...
                console.warn(`[pantheon/session-notification] invalid AV_PANTHEON_NOTIFY_DELAY_MS="${raw}"; using default ${DEFAULT_SESSION_NOTIFICATION_CONFIG.idleConfirmationDelayMs}ms`);
            }
        }
    }
    if (typeof env.AV_PANTHEON_NOTIFY_SOUND === "string") {
        ...
        if (raw === "1") {
            ...
            console.warn(`[pantheon/session-notification] unrecognized AV_PANTHEON_NOTIFY_SOUND="${raw}"; expected "1" to enable; treating as disabled`);
        }
    }
